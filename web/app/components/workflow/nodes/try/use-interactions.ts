...
export const useNodeTryInteractions = () => {
  const { t } = useTranslation()
  const store = useStoreApi()
  const { nodesMap: nodesMetaDataMap } = useNodesMetaData()

  const handleNodeTryRerender = useCallback((nodeId: string) => {
    const {
      getNodes,
      setNodes,
    } = store.getState()
  ...
  const handleNodeTryChildDrag = useCallback((node: Node) => {
  ...
  const handleNodeTryChildSizeChange = useCallback((nodeId: string) => {
    const { getNodes } = store.getState()
    const nodes = getNodes()
    const currentNode = nodes.find(n => n.id === nodeId)!
    ...
    if (parentId)
      handleNodeTryRerender(parentId)
  }, [store, handleNodeTryRerender])

  const handleNodeTryChildrenCopy = useCallback((nodeId: string, newNodeId: string, idMapping: Record<string, string>) => {
    const { getNodes } = store.getState()
    const nodes = getNodes()
  ...
  return {
    handleNodeTryRerender,
    handleNodeTryChildDrag,
    handleNodeTryChildSizeChange,
    handleNodeTryChildrenCopy,
  }
}
